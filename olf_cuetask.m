...
for n = 1 :  totalTrials
    ...
    if n > 1
        % time interval between current trial and the last one
        %         WaitSecs( iti(n) - rt_timeout - GetSecs + resp_end_time( n-1));
        WaitSecs( iti(n) - GetSecs + resp_end_time( n-1));
    else
        % for the fist trial, show the cross for a time of minimal
        % inter-trial interval, iti(1)
        WaitSecs( iti(n));
    end
...
%     DaqDOut( daq(1), daq_dport, 0);
    ...
    % close olfactometer
%      WaitSecs(1.5); % TODO check how long the olfactometer need to stay open 
%      OlfOpenLines(1,daq(2));
    ...
    %%% button response 
        ...
        boxsize = [100, 80];  
        leftboxx = rect(3) / 4 - boxsize(1) / 2; 
        rightboxx = 3 * rect(3) / 4 - boxsize(1) / 2;
        boxY = rect(4) / 2 +200;
...
%     end
    ...
    if n < totalTrials
        DrawFormattedText( w, ['Starting trial ', num2str(n+1)], 'center', 'center', textColor);
        Screen( 'Flip', w);
        WaitSecs(2); % wait time to start next trial 
    end
